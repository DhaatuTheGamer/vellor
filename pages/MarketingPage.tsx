...
const MagneticButton: React.FC<{ children: React.ReactNode; onClick: () => void; className?: string; rightIcon?: import('../types').IconName }> = ({ children, onClick, className, rightIcon }) => {
...
const founderStoryParagraphs: React.ReactNode[] = [
...
export const MarketingPage: React.FC<MarketingPageProps> = ({ onGetStarted }) => {
  ...
  return (
    <main className="min-h-screen bg-gray-50 dark:bg-primary text-gray-900 dark:text-gray-100 overflow-x-hidden font-sans custom-scrollbar">
      ...
      <section id="features" data-pomelli-section="features" data-crawler-intent="education" className="py-24 px-4 bg-white dark:bg-primary-dark/50 relative z-20">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { title: "No Subscriptions", desc: "Most tutoring software costs $30+ a month. Vellor is 100% free forever.", icon: "currency-dollar" as const },
              { title: "Lightning Fast", desc: "Built on React 19 and Vite. Feel the 60fps animations and instant interactions.", icon: "rocket" as const },
              { title: "Offline First", desc: "No internet? No problem. Install as a native app and manage your business anywhere.", icon: "bolt" as const }
            ].map((item, i) => (
              <motion.div 
                key={item.title}
                initial={{ opacity: 0, y: 20 }}
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="relative">
            ...
            <div className="grid md:grid-cols-4 gap-8 relative z-10">
              {[
                { step: 1, title: "Onboard Students", desc: "Add student details and set custom goals instantly.", icon: "user-plus" as const },
                { step: 2, title: "Log Lessons", desc: "Track hours, topics, and performance with a single click.", icon: "clock" as const },
                { step: 3, title: "Auto-Invoice", desc: "Generate PDFs and send WhatsApp reminders effortlessly.", icon: "document-text" as const },
                { step: 4, title: "Track Growth", desc: "Monitor revenue and earn gamified achievements.", icon: "trending-up" as const }
              ].map((item, i) => (
                <motion.div
                  key={item.step}
      ...
      <PremiumFeaturesSection data={data} />
              ...
              <div className="relative">
                <AnimatePresence initial={false}>
                  {isManifestoExpanded && (
                    ...
                    >
                      {founderStoryParagraphs.map((paragraph, idx) => (
                        <p key={idx} className="text-xl leading-relaxed text-gray-700 dark:text-gray-300 text-pretty">
                          {paragraph}
                        </p>
                      ))}
          ...
          <>
            ...
            >
              ...
              <div className="flex-1 flex flex-col gap-1 p-4 overflow-y-auto">
                {[
                  { label: 'Features', id: 'features', icon: 'sparkles' as const },
                  { label: 'Gamification', id: 'gamification', icon: 'trophy' as const },
                  { label: 'Privacy', id: 'privacy', icon: 'lock-closed' as const },
                  { label: 'Open Source', id: 'open-source', icon: 'code' as const },
                  { label: 'FAQ', id: 'faq', icon: 'question-mark-circle' as const },
                ].map((item) => (
                  <button
                    key={item.id}
