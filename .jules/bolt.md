...
## 2026-04-22 - Array map() and filter() in High-Frequency Renders and State Updates\n**Learning:** Replacing `Array.prototype.filter()` with pre-allocated `for` loops eliminates intermediate array allocations, reducing garbage collection pressure inside frequent render cycles. Additionally, when updating items in an array within Zustand state setters, unconditionally cloning the entire array (e.g., `const newItems = [...state.items]`) *before* iterating to find a match introduces unnecessary O(N) allocation overhead for non-matching updates. \n**Action:** When working on array transformations in performance-critical paths (like renders), use pre-allocated `for` loops instead of `.filter()`. When updating state arrays, iterate over the current array and only create a shallow copy *after* a match is found.
